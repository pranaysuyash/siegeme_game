...
export function evaluateDodoPayment(event: Record<string, unknown>, intent: PurchaseIntentRecord | null, duplicate: boolean): DodoGrantDecision {
  ...
  if (!intent || intent.status === "FAILED" || !["ATTACK_PACK", "DEFENSE_PACK"].includes(intent.purchase_kind) || productId !== intent.expected_product_id || totalAmount !== intent.expected_amount_minor || currency?.toUpperCase() !== intent.expected_currency.toUpperCase()) {
