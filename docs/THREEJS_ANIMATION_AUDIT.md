...
| A-05 mobile attacker affordance | **Resolved locally for the active attack surface** | `scripts/browser-attack-flow.mjs` now claims real Worker-backed turns at 390×844 and 844×390, asserts the canvas, live metadata row, attack HUD, readout, and release control remain visible within the viewport, and exercises pointer/visibility cancellation plus synthetic context recovery. The fixture captures the portrait and landscape attack images and records measured rectangles and interruption results in `attack-flow.json`. This is local Tier 4 browser evidence, not real-device proof. |
