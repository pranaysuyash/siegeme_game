...
**Audit Standard:** [threejs-fundamentals Skill](file:///Users/pranay/Projects/skills/3d-web/threejs-fundamentals/SKILL.md)
**Date:** August 27, 2026
**Audited Targets:**
...
| **Shadow Map Mode** | Default (`THREE.PCFShadowMap` via `shadows`) | `THREE.PCFSoftShadowMap` | ⚠️ **Opportunity** — Enabling PCF Soft shadows (`gl.shadowMap.type = THREE.PCFSoftShadowMap`) creates smoother penumbras for fortress battlements. |
...
- **Finding:** In [`GameCanvas.tsx#L215-L229`](file:///Users/pranay/Projects/siegeme_game/src/components/GameCanvas.tsx#L215-L229) (`Projectile`), `new THREE.Vector3()` is instantiated 3 times per frame during projectile flight:
  ```typescript
...
- **Finding:** [`ballistics.ts`](file:///Users/pranay/Projects/siegeme_game/src/game/simulation/ballistics.ts) implements analytical point-in-box ray/swept-AABB tests using pure numerical tuples `[number, number, number]` without importing `three` or allocating `THREE.Vector3` objects on the Cloudflare Worker runtime.
...
## 5. Prioritized Action Summary

1. **P0 (Hot Path GC):** Replace inline `new THREE.Vector3()` in `Projectile` with static/cached Vector3 math.
2. **P1 (Visual Quality):** Set `gl.shadowMap.type = THREE.PCFSoftShadowMap` in Canvas `onCreated` for softer shadow penumbras.
3. **P2 (Memory & Draw Calls):** Group repeated identical crenellation geometries into shared geometry instances.
