...
- one named Cloudflare Durable Object for canonical current-world authority and persistence; D1 for durable ledgers/archive.
- Durable Object Hibernating WebSockets for realtime fanout.
...
- Do not build a login wall.
- Silently issue a stable server-side player_id + signed session on first meaningful interaction.
- Dodo webhook ties paid entitlements to that player_id.
- Same-device play uses the signed session.
- During coronation, strongly prompt/require recovery setup for cross-device ruler control and notifications.
- Prefer email magic link first; no passwords required initially.
- Keep public throne identity separate from player/account identity.
- Read `implementation/ARCHITECTURE_AND_PLAYER_IDENTITY_CORRECTION.md` before implementing auth/session behavior.
