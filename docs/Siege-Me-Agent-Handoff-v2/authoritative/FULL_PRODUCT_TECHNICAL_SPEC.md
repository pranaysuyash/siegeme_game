...
The live authority is **Cloudflare-first**: one named Durable Object owns the current global throne/world, D1 stores durable ledgers/archive data, R2 stores blobs, and Dodo remains payment authority. The first production world is designed to be **procedural-first 3D and capable of shipping with zero external model-pack dependency**. This does not mean reproducing ornate concept-art castles entirely from primitives. Three.js/React Three Fiber render a deliberately stylized modular world; Rapier supplies physics; a deterministic semantic world generator maps directly to persistent server state. Authored 3D assets remain optional later upgrades.
...
## 26.1 Web app
- Next.js + TypeScript + React if the existing implementation has already started there.
- Frontend hosting is deliberately decoupled from game authority.
- Vercel is acceptable for the shell.
- Cloudflare Workers is also viable for the shell; current Cloudflare guidance recommends `vinext` for new Next.js-on-Workers deployments, but that remains a deployment-adapter choice rather than a game-architecture dependency.

Do not make the live game authority depend on Vercel serverless functions.
...
Use it for UI state, local snapshots, and ephemeral interaction state only.
Do not put per-frame transforms in React state.
Do not treat Zustand as authoritative world state.

## 26.5 Live game authority
- **Cloudflare Durable Objects**
- One named SQLite-backed Durable Object is the source of truth for the one global Siege Me world/throne.

Recommended conceptual binding:
- class: `SiegeWorld`
- canonical object ID/name: `global-throne-v1`

The Durable Object owns:
- current reign;
- ruler/public identity reference;
- world-state version;
- Core Integrity;
...
- current attack turn;
- attack queue;
- live entitlement balances required for play;
- Siege Charge;
- Royal Guard Charge;
- succession lock/state;
- connected WebSocket spectators;
- canonical live-event sequence.

Important current state must be persisted in the Durable Object's attached SQLite storage; in-memory state is only an optimization.

## 26.6 Realtime
- **Durable Object WebSockets**
- Prefer Cloudflare's WebSocket Hibernation API.

All spectators/players for the one global throne connect to the same authoritative Durable Object.

The Durable Object accepts WebSockets, broadcasts canonical events, serializes mutations, can hibernate while idle spectators remain connected, and rehydrates durable state after wake/restart.

## 26.7 Durable ledger / query database
- **Cloudflare D1**

D1 is not the live throne authority.

Use D1 for:
- Dodo webhook/idempotency records;
- durable payment ledger;
- public identity submissions/metadata;
- historical reign archive;
- historical contribution/query tables;
- moderation cases;
- audit/event archive;
- recovery/reconciliation records.

Live shots must not require a D1 round trip to decide who owns the throne or whether a state mutation wins a race.

## 26.8 Object storage
- **Cloudflare R2**

Use R2 for normalized ruler logos/avatars, persisted share-card renders if needed, moderation evidence where appropriate, and optional replay/diagnostic blobs later.

No required 3D model storage is needed for the first procedural world.

## 26.9 Cache / configuration
- Workers KV is optional.
- KV may cache public read-mostly data, configuration, feature flags, generated summaries, or non-authoritative metadata.
- **KV must never decide current ruler, Core health, paid entitlement validity, turn ownership, or victory.**

## 26.10 Background work
- Cloudflare Queues optional but recommended for retryable asynchronous side effects such as archival, analytics delivery, moderation jobs, and payment-to-world grant reconciliation.

The primary live mutation path remains Durable Object-local.
...
Payment flow:
1. Dodo webhook reaches a Cloudflare Worker endpoint.
2. Worker verifies signature and idempotency.
3. Worker records provider/payment/entitlement ledger in D1.
4. Worker sends an idempotent grant command to the global SiegeWorld Durable Object.
5. Durable Object persists the live entitlement grant in its own strongly consistent storage.
6. If the downstream grant fails after payment was accepted, recovery/reconciliation retries from D1 and/or Queue.

## 26.12 Hosting
Preferred launch split:
- game authority/API/WebSockets/Dodo webhook: Cloudflare Workers + Durable Objects;
- static/assets/uploads: Cloudflare/R2;
- UI shell: keep on Vercel if already established, or move to Cloudflare Workers/Pages when convenient.

Do not block the product on moving the shell between Vercel and Cloudflare.

## 26.13 Analytics
- PostHog later/at launch depending effort.
- Analytics must not sit in the critical mutation path.

## 26.14 Error monitoring
- Sentry or equivalent.
- Cloudflare logs/analytics for Worker/DO operational visibility.
...
Exact repository structure may differ, but renderer, domain, generator, simulation, Cloudflare authority, and payment code must be separable.
...
### Simulation
Knows deterministic/authoritative projectile and damage rules.

### Cloudflare Worker facade
Knows HTTP routing, Dodo webhook ingress, asset endpoints, snapshots and Durable Object routing.

### SiegeWorld Durable Object
Owns all concurrency-sensitive current world truth.

### D1
Stores durable ledgers/archive/queryable historical data, not live ownership truth.

### R2
Stores blobs, not game authority.

### KV
Optional cache/config only.

### Payment
Creates verified entitlement grants, never damage.

## 27.3 Critical-path rule

A live shot should conceptually be:

`client input -> Worker/DO route -> SiegeWorld DO -> validate turn + entitlement -> simulate/verify -> mutate DO storage atomically -> broadcast canonical event`

It should **not** be:

`client -> stateless API -> D1 query -> separate realtime service -> database update -> race to determine winner`.

## 27.4 One global object now, sharding later only if product changes

The launch product has one global throne, therefore one global Durable Object is the natural authority.

Do not introduce geographic/world shards preemptively.

If the product later adds multiple independent worlds/rooms, map each independent world to a different Durable Object ID.
...
# 30. Durable Data Design

There are two durable state layers with different responsibilities.

## 30.0 Authority split

### SiegeWorld Durable Object SQLite storage
Stores the concurrency-sensitive current state:
- current reign/ruler;
- current public identity snapshot/reference;
- world seed/generator version;
- state version;
- Core Integrity;
- persistent component state;
- shields/braces;
- active/pending turn queue;
- live entitlement balances;
- shared meters;
- succession state;
- compact recent event sequence.

### D1
Stores queryable durable ledger/archive data:
- players/session recovery records where required;
- public identities;
- reign archive/summaries;
- payment and webhook idempotency;
- entitlement ledger;
- archived shots/events;
- contribution/history tables;
- moderation cases.

D1 may lag live presentation slightly when archival is asynchronous. The Durable Object may not lag for current ownership/turn/victory decisions.

The field lists below are logical schemas. D1 and Durable Object SQLite should use SQLite-compatible types such as `TEXT`, `INTEGER`, `REAL`, `BLOB`, and JSON encoded as `TEXT` where appropriate.
...
- `primary_email_hash TEXT NULL`
- `status TEXT`
- `last_seen_at INTEGER`

## 30.2 `public_identities` (D1)
- `id TEXT PRIMARY KEY`
...
## 30.3 `reign_archive` (D1)
- `id TEXT PRIMARY KEY`
- `ordinal INTEGER UNIQUE`
- `ruler_player_id TEXT`
- `public_identity_id TEXT NULL`
- `started_at INTEGER`
- `ended_at INTEGER`
- `world_seed TEXT`
- `generator_version TEXT`
- `final_state_version INTEGER`
- `starting_core_integrity INTEGER`
- `ending_core_integrity INTEGER`
- `conqueror_player_id TEXT NULL`
- `archive_summary_json TEXT`
- `archived_at INTEGER`

The active reign is authoritative in the SiegeWorld DO.

## 30.4 `world_components` (Durable Object active state; optional D1 archive)
- `component_id TEXT`
- `component_type TEXT`
- `hp INTEGER`
- `max_hp INTEGER`
...
- `version INTEGER`
- `destroyed_event_id TEXT NULL`

## 30.5 `defense_placements` (Durable Object active state; D1 archive optional)
- `id TEXT`
- `reign_id TEXT`
- `player_id TEXT`
- `entitlement_id TEXT`
...
- `status TEXT`
- `created_at INTEGER`
- `granted_to_world_at INTEGER NULL`
- `reconciled_at INTEGER NULL`

## 30.7 `live_entitlements` (Durable Object)
- `entitlement_id TEXT PRIMARY KEY`
- `player_id TEXT`
- `kind TEXT`
- `quantity_granted INTEGER`
- `quantity_remaining INTEGER`
- `grant_source_id TEXT UNIQUE`
- `status TEXT`
- `created_at INTEGER`

Grant and consume commands are idempotent.

## 30.8 `attack_turns` (Durable Object active state; D1 archive optional)
- `id TEXT`
- `reign_id TEXT`
- `entitlement_id TEXT`
- `player_id TEXT`
- `queue_position_seq INTEGER`
- `status TEXT`
- `shots_total INTEGER`
- `shots_used INTEGER`
- `lease_started_at INTEGER NULL`
- `lease_expires_at INTEGER NULL`
- `bound_world_version INTEGER`
- `bound_world_hash TEXT`
- `created_at INTEGER`
- `completed_at INTEGER NULL`

## 30.9 `shots` (Durable Object recent state + D1 archival)
- `id TEXT`
- `turn_id TEXT`
- `reign_id TEXT`
- `player_id TEXT`
- `ordinal INTEGER`
- `world_version_before INTEGER`
- `world_version_after INTEGER`
- `input_json TEXT`
...
The Durable Object maintains the canonical live sequence. D1 stores append-only archive rows.
...
## 30.13 Archival strategy

Do not synchronously write every live visual event to D1 before broadcasting.

Preferred:
1. Durable Object commits live authoritative mutation.
2. Durable Object broadcasts canonical event.
3. Important recent event remains in DO storage.
4. D1 archival happens asynchronously or in bounded batches, optionally through Cloudflare Queues.
5. At reign end, write/verify a complete reign summary/archive.

Payment/idempotency records are different: D1 should durably record the verified payment before/with world entitlement grant/reconciliation logic.

---

# 31. Atomic World Commit

## 31.1 Need

A shot can change component HP, shield, Core, shared meter, entitlement remaining, turn status, reign status, and succession state.

These changes must have one serialized authority.

## 31.2 Recommended approach

The named `SiegeWorld` Durable Object is the mutation boundary.

For every authoritative command:
1. verify caller/session/command token;
2. verify expected reign;
3. verify expected world state version;
4. verify active turn/placement window;
5. verify live entitlement/resource;
6. run deterministic authoritative simulation or validation;
7. update all affected live state in the Durable Object's strongly consistent SQLite storage;
8. increment world/event sequence/version;
9. consume entitlement only as part of the accepted mutation;
10. persist defeat/succession state in the same serialized authority;
11. broadcast sanitized result after commit.
...
- do not consume shot unless accepted.

## 31.4 Idempotency

Every paid/action command has a stable command or shot ID.

Replaying the same accepted command returns the prior authoritative result and does not consume/apply again.
...
## 32.1 Transport
Durable Object WebSockets, preferably using the Hibernation API.

All clients observing the single global throne connect to the one authoritative `SiegeWorld` object.

## 32.2 Direction
Clients may send session handshakes, authorized gameplay input commands and resync requests.

Clients must not send trusted:
- damage;
- Core health;
- ruler changes;
- entitlement grants;
- final victory;
- arbitrary broadcast events.

The Durable Object emits sanitized canonical events.

## 32.3 Event model

```ts
type WorldEvent =
  | { type: "SNAPSHOT_VERSION"; version: number; sequence: number }
  | { type: "TURN_STARTED"; version: number; sequence: number; turnId: string; actor: PublicActor }
  | { type: "SHOT_RESOLVED"; version: number; sequence: number; eventId: string; summary: ShotSummary }
  | { type: "DEFENSE_PLACED"; version: number; sequence: number; placement: DefenseSummary }
  | { type: "REIGN_DEFEATED"; version: number; sequence: number; conqueror: PublicActor }
  | { type: "REIGN_STARTED"; version: number; sequence: number; ruler: PublicIdentitySummary };
```

Do not broadcast the full snapshot for every mutation.

## 32.4 Hibernation
Use WebSocket Hibernation so idle spectators can remain connected without requiring the object to stay actively billed for idle duration.

Persist all correctness-critical state in DO storage.

## 32.5 Recovery
On reconnect, sequence gap, stale version, app resume, DO restart or WebGL recovery, fetch/request the latest authoritative snapshot then resume the event stream.

## 32.6 Presence
Viewer count is approximate/social and must never affect Core HP or combat power.

---

# 33. Authentication and Session Model

Siege Me does not require a full user-account product to play.

## 33.1 Spectator/session identity
On first useful interaction, issue an opaque player/session identifier.

Recommended launch model:
- signed secure HTTP-only cookie where same-origin architecture permits;
- otherwise short-lived signed session token from the Cloudflare Worker;
- server maps token to an internal player/session ID.

No signup UI is required to spectate.

## 33.2 Paid player
Before checkout, ensure a stable internal player/session ID exists.

Verified payment associates:
- Dodo payment/customer identity;
- D1 payment record;
- internal player/session ID;
- entitlement grant ID.

No card data is stored.

## 33.3 Cross-device recovery
Keep recovery deliberately small at launch.

Possible later/when-needed recovery:
- email magic link through a transactional email provider;
- verified email from a payment/customer flow where permitted;
- explicit account linking.

Do not add a full auth platform merely to solve a launch problem that does not exist.

## 33.4 Conqueror
The conqueror already has a stable session/player ID because they used a paid entitlement.

During coronation:
- associate the public identity with that player ID;
- optionally require stronger verification for high-risk/famous-brand outbound identity;
- fallback identity may be display-name-only until moderation/verification succeeds.

## 33.5 Authorization principle
Authorization lives in Worker/Durable Object server code.
A browser token never grants direct storage access.
...
## 47.1 Secrets
Never expose:
- Cloudflare API/account tokens;
- privileged bindings/secrets;
- Dodo secret;
- Dodo webhook signing secret;
- session-token signing keys.

## 47.2 Browser boundary
The browser may receive public API/WebSocket endpoints and intended public/session data, but never credentials that allow direct authoritative storage mutation.

## 47.3 Durable Object command security
Every mutation validates:
- session/player;
- active turn/placement permission;
- entitlement;
- reign/world version;
- idempotency key;
- rate limits.

Do not expose a generic `setState` command.

## 47.4 D1
D1 is accessed through trusted Worker code.
Do not expose raw D1 SQL/data access to clients.
Use uniqueness constraints for webhook/payment/idempotency-critical IDs.

## 47.5 KV
KV is eventually consistent and optional.
Never use KV for ownership, Core health, paid entitlement truth, turn locks or victory.

## 47.6 Uploads / R2
Normalize uploads. Validate MIME type, decoded image content, size/dimensions and moderation status.
Do not serve active HTML/SVG/script content as ruler media initially.

## 47.7 URLs
HTTPS only initially.
Normalize destination URLs, show destination domain, and run phishing/malware/reputation checks appropriate to risk.

## 47.8 Rate limits and abuse
Rate limit checkout creation, WebSocket churn, queue/turn requests, shot retries, defense placement, identity edits/uploads and recovery endpoints.

Have an emergency kill switch for purchases, attacks, identity publishing and outbound links.

## 47.9 Cost protection
- Cloudflare spend notifications/alerts;
- rate limit unauthenticated endpoints;
- bound WebSocket message frequency;
- use Hibernation;
- avoid high-frequency presence broadcast;
- batch/archive to D1 rather than writing every render detail;
- limit R2 uploads;
- monitor DO request/duration/storage and D1 reads/writes;
- prefer Workers Paid at public launch if hard Free-tier limits would otherwise turn a viral spike into errors.
...
- Durable Object WebSockets is currently recommended by Supabase for scalable/security-conscious fan-out compared with Postgres Changes for large subscriber counts.
