...
**Audit Standard:**
- Primary: `~/Projects/skills/threejs-geometry/SKILL.md`
- Secondary: `~/Projects/external-skills/full-stack-skills__threejs-skills/skills/threejs-geometries/SKILL.md`
...
The current procedural world rendering in [`GameCanvas.tsx`](../src/components/GameCanvas.tsx) utilizes declarative built-in Three.js primitives:
...
- **Current Pattern:** Unique architectural primitives remain declarative JSX, while repeated crenellations and rubble use Drei `Instances`/`Instance` batches.
- **Behavior:** R3F owns the lifecycle of declarative geometries, and Drei shares repeated geometry/material through one instanced draw path.
- **Finding:** The high-count repeated geometry issue is resolved for the two repeated families. Unique low-poly architectural blocks remain explicit for readable semantic component boundaries.

### B. Instancing & `InstancedMesh` / `InstancedBufferGeometry` Opportunities
- **Audited Target:** `Crenellations` and `RubbleFragments` (`GameCanvas.tsx`):
  - Both repeated families now use a single Drei `Instances` container with shared geometry/material and per-instance transforms.
  - This collapses repeated decorative fragments to one instanced draw path per family while preserving semantic component meshes for authority and effects.
...
1. **Resolved (Buffer Reuse via Instancing):**
   - `Crenellations` and destroyed rubble use Drei `Instances` with shared geometry/material.
