...
export function impactLabel(targetId: string, damage: number, projectileType: "STANDARD" | "BREAKER" = "STANDARD", snapshot: PublicWorldSnapshot | null = null, defenseType?: "SHIELD" | "BRACE") {
  ...
  if (targetId.startsWith("defense:")) {
    ...
    return `${prefix}${defenseType === "BRACE" || defense?.type === "BRACE" ? "Brace held" : "Shield absorbed"}${damage > 0 ? ` · −${damage}` : ""}`;
  }
