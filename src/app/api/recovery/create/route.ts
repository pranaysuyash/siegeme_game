...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(new URL("/recovery/create", authorityUrl), { method: "POST", headers: { ...(request.headers.get("cookie") ? { Cookie: request.headers.get("cookie") as string } : {}) }, cache: "no-store" });
