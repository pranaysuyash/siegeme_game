...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(new URL("/defense/place", authorityUrl), { method: "POST", headers: { "Content-Type": "application/json", ...(request.headers.get("cookie") ? { Cookie: request.headers.get("cookie") as string } : {}) }, body: JSON.stringify(await request.json()), cache: "no-store" });
